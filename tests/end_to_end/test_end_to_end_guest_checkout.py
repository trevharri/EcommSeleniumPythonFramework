...
class TestEndToEndCheckoutGuestUser:
    ...
    def test_end_to_end_checkout_guest_user(self):
        ...
        log.debug("Waiting for item count in header to update")
        header.wait_until_cart_item_count(1)
        log.debug("Clicking on left cart button in navigation bar")
        header.click_on_nav_cart()

        log.debug("Getting list all product names in cart")
        ...
        log.info(f"Applying coupon code: {coupon_code}")
        ...
        log.debug("Going to checkout page")
        cart_page.go_to_checkout()

        log.debug("Filling in billing info")
